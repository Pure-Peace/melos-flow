import MelosFee from "MelosFee.cdc"
...
pub contract MelosOrder {
    ...
    pub fun addOrder(
        ...
        let addPayment = fun (type: String, address: Address, rate: UFix64) {
            ...
            percentage = percentage - (type == MelosOrder.BUYER_FEE ? 0.0 : rate)
            assert(rate >= 0.0 && rate < 1.0, message: "Sum of payouts must be in range [0..1)")
        }

        addPayment(MelosOrder.BUYER_FEE, MelosFee.feeAddress(), MelosFee.buyerFee)
        addPayment(MelosOrder.SELLER_FEE, MelosFee.feeAddress(), MelosFee.sellerFee)

        for cut in extraCuts {
            addPayment(MelosOrder.OTHER, cut.address, cut.rate)
        }

        for royalty in royalties {
            addPayment(MelosOrder.ROYALTY, royalty.address, royalty.rate)
        }

        addPayment(MelosOrder.REWARD, orderAddress, percentage)
