...
pub contract MelosEnglishAuction {
    ...
    pub resource Auction {
        ...
        init(
            ...
            pre {
                duration >= MelosEnglishAuction.minimalDuration
                minimumBid >= MelosEnglishAuction.reservePrice
                increment >= MelosEnglishAuction.reservePrice
        ...
        pub fun appendBid(mark: UInt64, bid: @Bid): @Bid? {
            ...
            if timestamp + MelosEnglishAuction.minimalDuration > self.finishAt! {
                self.finishAt = timestamp + MelosEnglishAuction.minimalDuration
            }
    ...
    pub resource AuctionManager {
        ...
        pub fun borrow(auctionId: UInt64): &Auction? {
            return &MelosEnglishAuction.auctions[auctionId] as? &Auction
        }
        ...
        pub fun createLot(
            ...
            MelosEnglishAuction.auctions[auctionId] <-! auction
            return auctionId
        }

        pub fun cancelLot(auctionId: UInt64) {
            let auction <- MelosEnglishAuction.auctions.remove(key: auctionId)!
        ...
        pub fun createBid(
            ...
            let auction = &MelosEnglishAuction.auctions[auctionId] as? &Auction
    ...
    pub fun borrow(auctionId: UInt64): &Auction? {
        if MelosEnglishAuction.auctions.containsKey(auctionId) {
            return &MelosEnglishAuction.auctions[auctionId] as! &Auction
        } else {
            return nil
        }
    ...
    pub fun auctionIDs(): [UInt64] {
        return MelosEnglishAuction.auctions.keys
    }

    pub fun unclaimedBidsIDs(): [UInt64] {
        return MelosEnglishAuction.unclaimedBids.keys
    }

    pub resource Admin {
        pub fun setMinimalDuration(value: UFix64) {
            if (value != MelosEnglishAuction.minimalDuration) {
                emit PropertyUpdated(name: "minimalDuration", value: value)
                MelosEnglishAuction.minimalDuration = value
            }
        }

        pub fun setMaximalDuration(value: UFix64) {
            if (value != MelosEnglishAuction.maximalDuration) {
                emit PropertyUpdated(name: "maximalDuration", value: value)
                MelosEnglishAuction.maximalDuration = value
            }
        }

        pub fun setReservePrice(value: UFix64) {
            if (value != MelosEnglishAuction.reservePrice) {
                emit PropertyUpdated(name: "reservePrice", value: value)
                MelosEnglishAuction.reservePrice = value
            }
        }
    }
